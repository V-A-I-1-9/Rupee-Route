...
$stmt = $conn->prepare("UPDATE expenses SET date = ?, category_id = ?, amount = ?, payment_method = ?, description = ?, investment_id = ? WHERE id = ?");
 // Adjust bind_param types: s=string, d=double/decimal, i=integer
$stmt->bind_param("ssdssii", $date, $category_id, $amount, $payment_method, $description, $investment_id, $id);
